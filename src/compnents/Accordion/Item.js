...
export default function Item({ name, id, child, active, toggle, children }) {
    ...
    return (
        <div className="bg-white border border-gray-300 overflow-hidden border-unrun">
            <div className="meta pl-4 py-2 relative flex justify-between item-center">
                ...
                <button className="link-wrapped pr-5 focus:outline-none" onClick={() => toggle(id)}>
                    {
                        child && child.length > 0 && <ArrowDown className={['transition-all duration-300 transform', active === id ?
                        "rotate-180" : "rotate-0"].join(" ")}></ArrowDown>
                    }
                </button>
            </div>
